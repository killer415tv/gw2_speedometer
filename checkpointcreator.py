    ...
    def on_press(self,key):
        ...
        try:
            if key.char == "1":
                ...
                writer.seek(0,2)
                writer.writelines( (',').join(["STEP","STEPNAME","X","Y","Z"]))
                writer.writelines("\r")
                writer.writelines( (',').join([str(order),"start",str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2])]))
                order = order + 1
                writer.close()
            ...
            if key.char == "2":
                ...
                writer.seek(0,2)
                writer.writelines("\r")
                writer.writelines( (',').join([str(order), "*", str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2])]))
                order = order + 1
                writer.close()
                self.file_ready = False
            ...
            if key.char == "3":
                ...
                writer.seek(0,2)
                writer.writelines("\r")
                writer.writelines( (',').join([str(order), "end", str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2])]))
                writer.close()
                self.file_ready = False
            ...
            if key.char == "4":
                ...
                writer.seek(0,2)
                writer.writelines("\r")
                writer.writelines( (',').join([str(-1), "reset", str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2])]))
                writer.close()
                self.file_ready = False

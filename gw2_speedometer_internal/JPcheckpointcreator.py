...
class Ghost3d(object):
    def on_press(self,key):
        ...
        try:
            if key.char == "1":
                order = 0
                self.best_file = os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "maps", "NEW.csv")
                writer = open(self.best_file, 'w',newline='', encoding='utf-8')
                writer.seek(0,2)
                writer.writelines( (',').join(["STEP","STEPNAME","X","Y","Z","RADIUS"]))
                writer.writelines("\r")
                writer.writelines( (',').join([str(order),"start",str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2]),"5"]))
                order = order + 1
                writer.close()
            ...
            if key.char == "2":
                ...
                writer.seek(0,2)
                writer.writelines("\r")
                writer.writelines( (',').join([str(order), "*", str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2]),"5"]))
                order = order + 1
                writer.close()
                self.file_ready = False
            ...
            if key.char == "3":
                ...
                writer.seek(0,2)
                writer.writelines("\r")
                writer.writelines( (',').join([str(order), "end", str(fAvatarPosition[0]),str(fAvatarPosition[1]),str(fAvatarPosition[2]),"5"]))
                writer.close()
                self.file_ready = False
...
#self.label.setFont(QtGui.QFont('Lucida Console', 20))
    ...
    def update(self):
        ...
        file = open(os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "checkpoint.txt"))
        filedata = file.read()
        if filedata == '': 
            return 
        ...
        if hasattr(self, 'df') and self.file_ready == True:
            ...
            if len(data) > 0:
                ...
                for p in points:
                    ...
                    self.md = gl.MeshData.cylinder(rows=1, cols=40, radius=[5 * 0.666,5 * 0.666], length=0.4)

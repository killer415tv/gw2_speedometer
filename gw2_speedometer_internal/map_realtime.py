...
def get_best_online_time(map_name):
    """Obtiene automáticamente el mejor tiempo online del ranking"""
    try:
        url = "https://www.beetlerank.com/api/top3/" + quote(map_name)
        response = requests.get(url, verify=True)
        data_json = json.loads(response.text)
        
        if data_json['ranking'] and len(data_json['ranking']) > 0:
            best_record = data_json['ranking'][0]  # El primer lugar es el mejor tiempo
            print("-----------------------------------------------")
            print(f"- MEJOR TIEMPO ONLINE: {best_record['time']} por {best_record['name']}")
            print(f"- ARCHIVO: {best_record['file']}")
            print("-----------------------------------------------")
            return best_record['file']
        else:
            print("No se encontraron rankings online para este mapa")
            return None
    except Exception as e:
        print(f"Error obteniendo ranking online: {e}")
        return None
...
class Ghost3d(object):
    ...
    def searchGhost(self):
        ...
        if forceFile:
            ...
            # Inicializar self.df al principio
            ...
            # Primero intentar obtener el mejor tiempo online
            best_online_file = get_best_online_time(guildhall_name)
            
            if best_online_file:
                # Cargar el archivo online
                try:
                    s = requests.get(best_online_file).content 
                    file_df = pd.read_csv(io.StringIO(s.decode('utf-8')))
                    file_df['file_name'] = best_online_file
                    
                    # Inicializar self.df si no existe
                    if not hasattr(self, 'df'):
                        self.df = pd.DataFrame()
                    
                    self.df = pd.concat([self.df, file_df], ignore_index=True)
                    self.best_file = best_online_file
                    print("-----------------------------------------------")
                    print("- LOADING ONLINE BEST TIME FOR MAP DRAWING")
                    print("-----------------------------------------------")
                    return
                except Exception as e:
                    print(f"Error loading online file: {e}")
                    print("Falling back to local files...")
            
            # Si no hay archivo online o falla, buscar en archivos locales
    ...
    def paintPlayer(self,name,x,y,color):
        ...
        self.canvas.create_oval(positionX+0, positionY+0, positionX+10, positionY+10, outline=color,fill=color, width=1, tags=namemd5+"marker")


if __name__ == '__main__':

    print("*****************************************************************")
    print("*             REALTIME MAP - ONLINE + OFFLINE VERSION           *")
    print("*****************************************************************")

...
class Racer():
    ...
    def open_multiplayer_map(self):
        if not self.mapOpen:
            self.session_id.get()
            subprocess.Popen(["python", Path(sys.argv[0]).parent.parent / "map_realtime_multiplayer.py", self.session_id.get()])
            self.mapOpen = True
    ...
    def saveGuildhall(self,value):
        ...
        if cup_name.get() == "OWN MAPS FOLDER":
            self.checkpoints_file = Path(sys.argv[0]).parent / "maps" / (guildhall_name.get() + ".csv")
        else:
            self.checkpoints_file = requests.utils.requote_uri('https://www.beetlerank.com/uploads/checkpoints/'+value+'.csv')
        try:

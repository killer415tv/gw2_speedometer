...
hud_max_speed = 0
magic_angle = (
    58  # angle for hud_angles_bubbles, to show a visual guide of the magic angle
)
...
loadfont(str(Path(sys.argv[0]).parent.parent / "font.ttf"))
loadfont(str(Path(sys.argv[0]).parent.parent / "fonts/Montserrat-Regular.ttf"))
...
class Configuration:
    ...
    def saveConf(self):
        ...
        f = open(Path(sys.argv[0]).parent / "config.txt", "w")
        cfg.write(f)
        f.close()
    ...
    def loadConf(self):
        ...
        if cfg.read([str(Path(sys.argv[0]).parent.parent / "config.txt")]):
            ...
            if cfg.has_option("general", "enable_livesplit_hotkey"):
                enable_livesplit_hotkey = int(
                    cfg.get("general", "enable_livesplit_hotkey")
                )
            ...
            if cfg.has_option("general", "show_checkpoints_window"):
                show_checkpoints_window = int(
                    cfg.get("general", "show_checkpoints_window")
                )
...
class Meterv2:
    ...
    def uploadLog(self, guildhall, old_filename, ml):
        ...
        response = requests.post(
            ...
            files={
                "file": open(Path(sys.argv[0]).parent / "logs" / old_filename, "rb")
            },
            headers=headers,
        )
    ...
    def __init__(self, master=None, **kw):
        ...
        # Verificar y corregir geometría antes de aplicarla
        screenW = self.root.winfo_screenwidth()
        screenH = self.root.winfo_screenheight()

        # Parsear geometría guardada
        if "x" in geometry_speedometer and "+" in geometry_speedometer:
            size_part, pos_part = geometry_speedometer.split("+", 1)
            x_pos, y_pos = map(int, pos_part.split("+"))
            win_w, win_h = map(int, size_part.split("x"))
            ...
            if y_pos < 0:
                y_pos = 10
            if x_pos + win_w > screenW:
                x_pos = screenW - win_w - 10
            if y_pos + win_h > screenH:
                y_pos = screenH - win_h - 10

            geometry_speedometer = f"{win_w}x{win_h}+{x_pos}+{y_pos}"
        ...
        if hud_angles_airboost:
            ...
            self.outer_airdrift_box = self.canvas.create_rectangle(
                20 + 306,
                80,
                30 + 306,
                150,
                outline="#666666",
                width="1",
                tags="airdrift_meter_border",
            )
            self.inner_airdrift_box = self.canvas.create_rectangle(
                23 + 306,
                83,
                27 + 306,
                147,
            ...
            self.airdrift_label = tk.Label(
                self.root,
                textvariable=self.airdrift_angle_tk,
                fg="white",
                bg="#666666",
                font=("Montserrat Regular", 9),
            ).place(x=17 + 306, y=152)
        ...
        if hud_angles_bubbles:
            ...
            self.canvas.create_circle(
                200, 300, 2, fill="#666", outline="white", width=1, tags="speed_angle"
            )
            self.canvas.create_circle(
                200, 300, 2, fill="#666", outline="white", width=1, tags="left_50_angle"
            )
            self.canvas.create_circle(
                200,
                300,
        ...
        if hud_drift_hold:
            ...
            self.outer_drifting_box = self.canvas.create_rectangle(
                20 + 45,
                80,
                30 + 45,
                150,
                outline="white",
                width="1",
                tags="drift_meter_border",
            )
            self.inner_drifting_box = self.canvas.create_rectangle(
                23 + 45,
                83,
                27 + 45,
                147,
            ...
            self.drifting_label = tk.Label(
                self.root,
                textvariable=self.drift_time_tk,
                fg="white",
                bg="#666666",
                font=("Montserrat Regular", 9),
            ).place(x=58, y=152)
        ...
        if hud_acceleration:
            self.acceltext = tk.Label(
                self.root,
                text="Accel.",
                fg="white",
                bg="#666666",
                font=("Montserrat Regular", 7),
            ).place(x=111, y=94)
            self.accelnum = tk.Label(
                self.root,
                textvariable=self.accelvar,
                fg="white",
                bg="#666666",
                font=("Montserrat Regular", 8, "bold"),
            ).place(x=111, y=110)
        ...
        if hud_speed:
            ...
            self.numero = tk.Label(
                self.root,
                textvariable=self.var100,
                fg="white",
                bg="#666666",
                font=("Montserrat Regular", 49, ""),
            ).place(anchor="center", x=200, y=115)

        if hud_angles:
            self.angletext = tk.Label(
                self.root,
                text="Cam    Beetle",
                fg="white",
                bg="#666666",
        ...
        if hud_altitude:
            ...
            self.altitudenum = tk.Label(
                self.root,
                textvariable=self.altitudevar,
                fg="white",
                bg="#666666",
                font=("Montserrat Regular", 10, "bold"),
            ).place(anchor="center", x=199, y=80)
        ...
        self.timenum_label = tk.Label(
            self.root,
            textvariable=self.vartime,
            fg="#eee",
            bg="#666666",
            font=("Digital-7 Mono", 20),
        ).place(x=144, y=155)
        ...
        self.steps0 = tk.Label(
            self.root,
            textvariable=self.steps_txt,
            fg="#fff",
            bg="#666666",
            font=("Lucida Console", 9, "bold"),
        ).place(anchor="center", x=200, y=242)
        self.step1_txt = tk.StringVar(self.root, "")
        self.steps1 = tk.Label(
            self.root,
            textvariable=self.step1_txt,
            fg="#eeeeee",
            bg="#666666",
            font=("Digital-7 Mono", 10),
        ).place(anchor="center", x=200, y=153)

        self.canvas.create_circle(
            200, 120, 96, fill="#666666", outline="#666666", width=2
        )
    ...
    def updateMeterTimer(self):
        ...
        if hud_drift_hold:
            ...
            if self.drifting:
                ...
                self.drift_time_tk.set(
                    round((time.perf_counter() - self.drift_time) * 10) / 10
                )
        ...
        def checkpoint(step, stepName, coords, radius):
            ...
            if step == -1:
                ...
                if distance.euclidean(_3Dpos, arraystep) < radius and (
                    pressedQ == 0 or different(last_checkpoint_position, arraystep)
                ):
                    if "racer" in globals():
            ...
            if step == next_step:
                ...
                if distance.euclidean(_3Dpos, arraystep0) < radius and (
                    pressedQ == 0 or different(last_checkpoint_position, arraystep0)
                ):
                    if "racer" in globals():
                        if stepName == "end":
                            racer.saveCheckpoint(0)
                        ...
                        if int(lap) == 1:
                            ...
                            self.step1_txt.set(
                                ...
                                + datetime.datetime.strftime(
                                    datetime.datetime.utcfromtimestamp(0), "%M:%S:%f"
                                )[:-3]
                            )
                            self.vartime.set("")
                            tmhud.write("")
                            ...
                            filename = (
                                guildhall_name.get() + "_log_" + str(_time) + ".csv"
                            )
                            if log:
                                # print("----------------------------------")
                                ...
                                # print("----------------------------------")
                                writer = open(
                                    Path(sys.argv[0]).parent / "logs" / filename,
                                    "a",
                                    newline="",
                                    encoding="utf-8",
                                )
                            ...
                            self.step1_txt.set(
                                ...
                                + " "
                                + datetime.datetime.strftime(
                                    datetime.datetime.utcfromtimestamp(steptime_lap),
                                    "%M:%S:%f",
                                )[:-3]
                            )
                    ...
                    if stepName == "end":
                        ...
                        if filename != "":
                            ...
                            if int(lap) == int(total_laps):
                                ...
                                file_df = pd.read_csv(
                                    Path(sys.argv[0]).parent / "logs" / old_filename
                                )
                                last_filename_df = pd.concat(
                                    [last_filename_df, file_df], ignore_index=True
                                )

                                current_time = last_filename_df.values[-1][6]
                                datefinish = datetime.datetime.strftime(
                                ...
                                if log:
                                    ...
                                    folder = str(
                                        Path(sys.argv[0]).parent / guildhall_name.get()
                                    )
                                    writer = open(
                                        folder + "_records.csv",
                                        "a",
                                    ...
                                    writer.writelines(
                                        (",").join(
                                            [
                                                datefinish,
                                                today_date,
                                                json.loads(ml.data.identity)["name"],
                                            ]
                                        )
                                    )
        ...
        if speed_in_3D:
            _pos = [
                ml.data.fAvatarPosition[0],
                ml.data.fAvatarPosition[1],
                ml.data.fAvatarPosition[2],
            ]
        else:
        ...
        if _lastTime + timer <= _time:
            ...
            # Arreglar bug de formas incompatibles cuando cambia speed_in_3D en caliente
            if len(_pos) != len(_lastPos):
                _lastPos = _pos.copy()
            dst = distance.euclidean(_pos, _lastPos)
            ...
            if True:
                ...
                if _pos[0] - _lastPos[0] == 0 or _pos[Y_index] - _lastPos[Y_index] == 0:
                    ...
                    if (
                        math.isnan(angle_between_res1) == False
                        and math.isnan(angle_between_res2) == False
                    ):
                        if hud_max_speed:
                        ...
                        if hud_angles:
                            self.anglevar.set(
                                str(int(angle_between_res1))
                                + "º/ "
                                + str(int(angle_between_res2))
                                + "º"
                            )
                        if hud_altitude:
                        ...
                        if hud_angles_bubbles:
                            ...
                            self.canvas.coords(
                                left_tick,
                                200 + 195 * float(r50v[0]) - 4,
                                195 + 195 * float(r50v[1]) - 4,
                                200 + 195 * float(r50v[0]) + 4,
                                195 + 195 * float(r50v[1]) + 4,
                            )
                            # representamos con dos circulos el angulo de velocidad y el de camara
                            ...
                            self.canvas.coords(
                                right_tick,
                                200 + 190 * float(l50v[0]) - 4,
                                195 + 190 * float(l50v[1]) - 4,
                                200 + 190 * float(l50v[0]) + 4,
                                195 + 190 * float(l50v[1]) + 4,
                            )
                            ...
                            self.canvas.coords(
                                speed_circle,
                                200 + 190 * float(uv[0]) - 4,
                                195 + 190 * float(uv[1]) - 4,
                                200 + 190 * float(uv[0]) + 4,
                                195 + 190 * float(uv[1]) + 4,
                            )
                            ...
                            self.canvas.coords(
                                camera_circle,
                                200 + 190 * float(uc[0]) - 8,
                                195 + 190 * float(uc[1]) - 8,
                                200 + 190 * float(uc[0]) + 8,
                                195 + 190 * float(uc[1]) + 8,
                            )
                            ...
                            self.canvas.coords(
                                avatar_circle,
                                200 + 190 * float(uaf[0]) - 11,
                                195 + 190 * float(uaf[1]) - 11,
                                200 + 190 * float(uaf[0]) + 11,
                                195 + 190 * float(uaf[1]) + 11,
                            )
                ...
                if filename != "" and round((velocity * 100 / 10000) * 99 / 72) < 150:
                    ...
                    self.vartime.set(
                        datetime.datetime.strftime(
                            datetime.datetime.utcfromtimestamp(_time - total_timer),
                            "%M:%S:%f",
                        )[:-3]
                    )
                    tmhud.write(
                        ...
                        writer.writelines(
                            (",").join(
                                [
                                    ...
                                    roundstr(_3Dpos[1]),
                                    roundstr(_3Dpos[2]),
                                    str(round((velocity * 100 / 10000) * 99 / 72)),
                                    roundstr(angle_between_res1),
                                    roundstr(angle_between_res2),
                                    str(_time - lap_timer),
                                    roundstr(acceleration),
                                    roundstr(map_angle),
                                ]
                            )
                        )
        ...
        if "racer" in globals() and racer.session_id.get() != "":
            ...
            if map_position_last_time_send != round(_time * 20):
                ...
                if not (
                    ml.data.fAvatarPosition[0] == 0
                    and ml.data.fAvatarPosition[1] == 0
                    and ml.data.fAvatarPosition[2] == 0
                ):
                    ...
                    if velocity > 0 or (
                        time.time() - last_time < 0.05 and last_valid > 0
                    ):
                    ...
                    if websocket_client:
                        south = np.array([0, -1])
                        avatar_direction = np.array(
                            [ml.data.fAvatarFront[0], ml.data.fAvatarFront[2]]
                        )
    ...
    def __init__(self, master=None, **kw):
        ...
        if hud_angles_airboost:
            ...
            self.airdrift_label = tk.Label(
                ...
                fg="white",
                bg="#666666",
                font=("Digital-7 Mono", 9),
            ).place(x=17 + 356, y=102)
        ...
        if hud_drift_hold:
            ...
            self.outer_drifting_box = self.canvas.create_rectangle(
                20, 30, 30, 100, outline="white", width="1", tags="drift_meter_border"
            )
            self.inner_drifting_box = self.canvas.create_rectangle(
                23,
                33,
        ...
        if hud_speed:
            ...
            ).place(relx=1, x=-412, y=73, anchor="ne")
        if hud_gauge:
            self.canvas.create_arc(
                2 * 10,
                2 * 15,
            ...
            if hud_max_speed:
                self.max_meter_meter = self.canvas.create_line(
                    winw, winw, 20, winw, fill="lime", width=4
                )
                self.max_speed.set(7250)
                self.updateMeterLine(0.5, self.max_meter_meter)
                self.max_speed.trace_add("write", self.updateMeterMaxSpeed)

            self.meter = self.canvas.create_line(
                winw, winw, 20, winw, fill="white", width=4
            )
            self.angle = 0.2
        ...
        self.steps0 = tk.Label(
            ...
            bg="#666666",
            font=("Lucida Console", 9, "bold"),
        ).place(anchor="center", x=204, y=202)
        self.step1_txt = tk.StringVar(self.root, "")
        self.steps1 = tk.Label(
            self.root,
            textvariable=self.step1_txt,
            ...
            bg="#666666",
            font=("Digital-7 Mono", 10),
        ).place(anchor="center", x=200, y=143)

        self.canvas.create_circle(
            204, 202, 171, fill="#666", outline="#666666", width=4
        )
...
class Racer:
    ...
    def on_message_MQTT(self, client, userdata, message):
        ...
        if received.get("option") == "f":
            ...
            self.timestamps.append(
                {"user": user, "time": time, "step": step, "lap": lap}
            )
            if str(step) == "999":
        ...
        if received.get("option") == "c":
            ...
            if step >= 1000:
                self.timestamps = list(
                    filter(lambda x: x["user"] not in [str(user)], self.timestamps)
                )
    ...
    def open_multiplayer_map(self):
        if not self.mapOpen:
            ...
            subprocess.Popen(
                [
                    "python",
                    Path(sys.argv[0]).parent.parent / "map_realtime_multiplayer.py",
                    self.session_id.get(),
                ]
            )
            self.mapOpen = True
    ...
    def update_countdown_label(self, first=False):
        ...
        if seconds_remaining > 1:
            ...
            delay = (
                math.floor((seconds_remaining - math.floor(seconds_remaining)) * 1000)
                if first
                else 1000
            )
    ...
    def joinRace(self):
        ...
        websocket_client = WebSocketApp(
            f"ws://{websocket_host}:{websocket_port}", on_message=on_message
        )
        ...
        conn_timeout = 5
        while (
            websocket_client.sock
            and not websocket_client.sock.connected
            and conn_timeout
        ):
            time.sleep(1)
            conn_timeout -= 1
    ...
    def toggleTrans(self):
        if self.move:
            ...
            self.websocket_race_status_label.configure(
                fg=self.color_trans_fg, bg=self.color_trans_bg
            )
            ...
            self.conf_websocket_host_entry.configure(
                fg=self.color_trans_fg, bg="#222222"
            )
            self.conf_websocket_port_entry.configure(
                fg=self.color_trans_fg, bg="#222222"
            )
            ...
            self.websocket_race_status_label.configure(
                fg=self.color_normal_fg, bg=self.color_normal_bg
            )
    ...
    def changeCup(self, value):
        ...
        if value == "OWN MAPS FOLDER":
            # Insert list of new options (tk._setit hooks them up to var)
            path = Path(sys.argv[0]).parent.parent / "maps"

            # get map list

            new_choices = [file.stem for file in path.glob("*.csv")]
            for choice in new_choices:
                self.t_3_2["menu"].add_command(
                    label=choice,
                    command=tk._setit(guildhall_name, choice, self.saveGuildhall),
                )
            ...
            if offline_mode == False:
                try:
                    ...
                    response = requests.get(
                        "https://www.beetlerank.com/api/maps/" + value, headers
                    )
                    ...
                    for choice in new_choices:
                        self.t_3_2["menu"].add_command(
                            label=choice,
                            command=tk._setit(
                                guildhall_name, choice, self.saveGuildhall
                            ),
                        )
    ...
    def saveGuildhall(self, value):
        ...
        # obtain the checkpoint file from website

        if cup_name.get() == "OWN MAPS FOLDER":
            self.checkpoints_file = (
                Path(sys.argv[0]).parent / "maps" / (guildhall_name.get() + ".csv")
            )
        else:
            self.checkpoints_file = requests.utils.requote_uri(
                "https://www.beetlerank.com/uploads/checkpoints/" + value + ".csv"
            )
        try:
        ...
        self.checkpoints_file = guildhall_name.get() + ".csv"
        ...
        if guildhall_name.get() == "None, im free!":
            ...
            try:
                headers = {"Origin": "null", "Referer": "null"}
                response = requests.get(
                    "https://www.beetlerank.com/rank/api/"
                    + str(guildhall_name.get())
                    + "/"
                    + self.username.get(),
                    headers,
                )
    ...
    def __init__(self):
        ...
        try:
            ...
            response = requests.get("https://www.beetlerank.com/api/cups", headers)
            resp_dict = response.json()
            ...
            self.cups = [
                directory.stem for directory in path.iterdir() if directory.is_dir()
            ]
        ...
        self.t_3 = tk.OptionMenu(
            self.root, cup_name, *self.cups, command=self.changeCup
        )
        ...
        self.t_3_2 = tk.OptionMenu(
            self.root, guildhall_name, *self.maps, command=self.saveGuildhall
        )
        ...
        def conf_save():
            ...
            # Arreglar restart para que funcione correctamente con venv
            import os
            ...
            if os.path.exists(venv_python):
                ...
                subprocess.Popen(
                    [venv_python, script_path], cwd=script_dir, shell=False
                )
            else:
                subprocess.Popen(["python", script_path], cwd=script_dir, shell=False)
            sys.exit()
        ...
        self.conf_move = tk.Button(
            self.root,
            text="MOVE SPEEDOMETER",
            command=lambda: toggleAll(),
            font=("Montserrat Regular", 10),
        )
        ...
        self.config_checkboxes = [
            (createCheckboxLabel(text), createCheckbox(init, varName))
            for (text, init, varName) in checkboxes
        ]

        # print(self.config_checkboxes[0][1].var)

        # COLOR
        self.conf_color_label = tk.Label(
            self.root,
            text="■ Player color",
            cursor="hand2",
            justify=tk.LEFT,
        ...
        self.conf_speedcolor0_label = tk.Label(
            self.root,
            text="■ [Gauge] Non-Driftable speed color <55",
            cursor="hand2",
            justify=tk.LEFT,
            padx=0,
            pady=0,
        ...
        self.conf_speedcolor0_label.bind(
            "<Button-1>", lambda e: choose_color("speed_color0")
        )
        self.conf_speedcolor1_label = tk.Label(
            self.root,
            text="■ [Gauge] Low speed color >55",
            cursor="hand2",
            justify=tk.LEFT,
            padx=0,
            pady=0,
        ...
        self.conf_speedcolor1_label.bind(
            "<Button-1>", lambda e: choose_color("speed_color1")
        )
        self.conf_speedcolor2_label = tk.Label(
            self.root,
            text="■ [Gauge] Medium speed color >70",
            cursor="hand2",
            justify=tk.LEFT,
            padx=0,
            pady=0,
        ...
        self.conf_speedcolor2_label.bind(
            "<Button-1>", lambda e: choose_color("speed_color2")
        )
        self.conf_speedcolor3_label = tk.Label(
            self.root,
            text="■ [Gauge] High speed color >83",
            cursor="hand2",
            justify=tk.LEFT,
            padx=0,
            pady=0,
        ...
        self.conf_speedcolor3_label.bind(
            "<Button-1>", lambda e: choose_color("speed_color3")
        )
        self.conf_speedcolor4_label = tk.Label(
            self.root,
            text="■ [Gauge] Faest speed color >100",
            cursor="hand2",
            justify=tk.LEFT,
            padx=0,
            pady=0,
        ...
        self.conf_speedcolor4_label.bind(
            "<Button-1>", lambda e: choose_color("speed_color4")
        )
        ...
        self.conf_websocket_host_entry = tk.Entry(
            self.root, textvariable=self.websocket_host, disabledbackground="#444444"
        )
        self.websocket_port = StringVar(self.root, str(websocket_port))
        self.conf_websocket_port_entry = tk.Entry(
            self.root, textvariable=self.websocket_port, disabledbackground="#444444"
        )
        ...
        # SAVE OPTIONS
        self.conf_save = tk.Button(
            self.root,
            text="SAVE & RESTART",
            command=lambda: conf_save(),
            font=("Montserrat Regular", 10),
        )
        ...
        def updateConfigVisibility():
            ...
            if show_config:
                ...
                self.conf_save.place(
                    x=320, y=54 + ((-1 + cb_counter) * 20), width=120, height=27
                )
                cb_counter += 1
            else:
                # ocultarlo
        ...
        self.conf = tk.Button(
            self.root,
            text="CONFIG",
            command=lambda: changeConfigVisibility(),
            font=("Montserrat Regular", "7"),
        )
        self.conf.place(x=259, y=44, width=45, height=15)
        ...
        self.t_7 = tk.Button(
            self.root,
            text="START RACE",
            command=lambda: self.newRaceThread(),
            font=("Montserrat Regular", 10),
        )
        self.t_7.place(x=200, y=120, width=100)
        self.t_7_1 = tk.Button(
            self.root,
            text="SURRENDER",
        ...
        self.websocket_race_status_label = tk.Label(
            self.root,
            textvariable=self.websocket_race_status,
            justify=tk.LEFT,
            fg=self.fg.get(),
            bg=self.bg.get(),
            font=("Montserrat Regular", 10),
        ...
        self.map_ranking = tk.Label(
            self.root,
            textvariable=self.map_ranking_var,
            justify=tk.LEFT,
            padx=0,
            fg=self.fg.get(),
            bg=self.bg.get(),
        ...
        self.t_4_6 = tk.Checkbutton(
            self.root,
            font=("Lucida Console", 10),
            text="",
            variable=upload,
            command=onClickUpload,
            borderwidth=0,
        )
        self.t_4_6.place(x=139, y=100)
        if not log:
    ...
    def listen_for_result(self):
        ...
        for n in uniqueNames:
            ...
            top_time = sorted(
                filtered_list,
                key=lambda k: (-int(k["lap"]), -int(k["step"]), k["time"]),
            )
        ...
        top_times = sorted(
            top_times, key=lambda k: (-int(k["lap"]), -int(k["step"]), float(k["time"]))
        )

        rankingtxt = ""
        rankingindex = 1
        for u in top_times:
            steptime = datetime.datetime.strftime(
                datetime.datetime.utcfromtimestamp(u["time"]), "%M:%S:%f"
            )[:-3]
            if str(u["step"]) == "999":
                rankingtxt = (
                    rankingtxt
...
class Countdown:
    ...
    def __init__(self):
        ...
        self.root.geometry(
            "{}x{}+{}+{}".format(windowWidth, windowHeight, positionRight, positionDown)
        )  # Whatever size
        ...
        self.time = tk.Label(
            self.root,
            textvariable=self.localcountdown,
            fg=self.fg.get(),
            bg=self.bg.get(),
            font=("Montserrat Regular", 60, "bold"),
        )
...
class Message:
    ...
    def __init__(self):
        ...
        self.outer_drifting_box = self.canvas.create_rectangle(
            1, 1, 499, 219, outline="white", width="1"
        )
        ...
        self.time = tk.Label(
            ...
            fg=self.fg.get(),
            bg=self.bg.get(),
            font=("Montserrat Regular", 17),
        )

        # self.time.place(x=0, y=0)
        self.btn = tk.Button(
            self.root,
            textvariable=self.timerStr,
            padx=20,
            font=("Lucida Console", 10),
...
class TrackManiaHud:
    ...
    def toggleTrans(self):
        if self.move:
            self.root.overrideredirect(1)
            self.time.configure(fg=self.color_trans_fg)
            self.time.configure(bg=self.color_trans_bg)
            self.root.configure(bg=self.color_trans_bg)
        else:
            self.root.overrideredirect(0)
            self.time.configure(fg=self.color_trans_fg)
...
if __name__ == "__main__":
    ...
    if show_checkpoints_window:
        ...
        try:
            ...
            print(
                "NO CONNECTION TO BEETLERANK.COM - Loading speedometer on OFFLINE mode"
            )
            print(
                "- You can put some folders on map folder as CUPS, and place the checkpoint files inside each CUP"
            )
